...
createServer(async (req, res) => {
  ...
  const file = path === '/' ? '/tests/concealment-browser.html' : path;
  if (file === '/vendor/three.module.js' || file === '/vendor/three.core.js') {
    const data = await readFile(resolve(root, 'node_modules/three/build', file.split('/').pop()));
    res.writeHead(200, {'Content-Type':'text/javascript','Cache-Control':'no-store'}).end(data); return;
  }
...
}).listen(5102, '127.0.0.1', () => console.log('Concealment regression fixture: http://127.0.0.1:5102'));
