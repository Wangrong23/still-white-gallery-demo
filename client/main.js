...
const sound=new Sound();let game=new Game({debug:true}),bot=new RehearsalBot(game),state=game.state,mode='menu',role='detective',chosenRole='detective',view={yaw:0,pitch:0},paused=false,keys=new Set(),aim=false,debug=false,showSpots=false,botEnabled=true,eventId=0,lastPhase='',phaseUntil=0,toastUntil=0,networkClock=0,started=false,waiting=false,lastTime=performance.now();
...
$('local').onclick=()=>{game=new Game({debug:true});bot=new RehearsalBot(game);state=game.state;begin('local',chosenRole);};
...
function lock(){if(document.pointerLockElement!==canvas)canvas.requestPointerLock()?.catch(()=>toast('点击画面后开始观察'));}
...
document.addEventListener('mousemove',e=>{if(document.pointerLockElement!==canvas||paused)return;view.yaw-=e.movementX*.0021;view.pitch=Math.max(-1.15,Math.min(1.15,view.pitch-e.movementY*.0018));});
...
document.addEventListener('keydown',e=>{if(!started||e.target.matches('input'))return;if(['Tab','Space','F2','F3','F4','F6','F7','F8','F9'].includes(e.code))e.preventDefault();if(e.repeat)return;if(e.code==='Escape'){if(!document.pointerLockElement)pause(!paused);return;}keys.add(e.code);if(paused)return;
 ...
 if(mode==='local'){if(e.code==='Tab'){game.input(role,{});role=role==='detective'?'killer':'detective';view={yaw:state.players[role].yaw,pitch:0};keys.clear();lastPhase='';toast(`控制 ${role.toUpperCase()}`);}if(e.code==='F2')debug=!debug;if(e.code==='F3')game.debugAction('time');if(e.code==='F4')game.debugAction('sunset');if(e.code==='F6')game.debugAction('night');if(e.code==='F7')game.debugAction('ammo');if(e.code==='F8')showSpots=!showSpots;if(e.code==='F9'){botEnabled=!botEnabled;game.input(role==='killer'?'detective':'killer',{});toast(botEnabled?'陪练已启用':'陪练已停止');}}
});
...
document.addEventListener('mousedown',e=>{if(document.pointerLockElement!==canvas||paused)return;if(e.button===0)action(role==='detective'?'shoot':'attack');if(e.button===2)aim=true;});document.addEventListener('mouseup',e=>{if(e.button===2)aim=false;});
...
function frame(now){const dt=Math.min((now-lastTime)/1000,.06);lastTime=now;
